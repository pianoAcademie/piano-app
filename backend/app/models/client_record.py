...
class AnnualSeriesTransferRequest(Base):
    ...
    target_session_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("course_sessions.id", ondelete="RESTRICT"), nullable=True)
    target_recurrence_group_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), nullable=True)
    desired_weekday: Mapped[int | None] = mapped_column(Integer, nullable=True)
    desired_time: Mapped[str | None] = mapped_column(String(5), nullable=True)
