...
def main() -> None:
    ...
    with SessionLocal() as db:
        ...
        execution = (followup.payload or {}).get(EXECUTION_KEY)
        if not isinstance(execution, dict) or str(execution.get("status") or "").lower() != "executed":
            print(f"[{SCRIPT_PREFIX}] abort=no_completed_enrollment_execution")
            db.rollback()
            return

        execution_student_id = str(execution.get("student_client_id") or "")
        if execution_student_id != str(TARGET_STUDENT_ID):
        ...
        if len(invoice_rows) != 1 or '"invoice_status":"CANCELLED"' in invoice_rows[0].message.replace(" ", ""):
        ...
        subscription_ids = _uuid_list(execution.get("created_subscription_ids"))
        if not booking_ids:
            print(f"[{SCRIPT_PREFIX}] abort=no_execution_bookings")
            db.rollback()
            return
        ...
        if len(subscriptions) != len(set(subscription_ids)):
