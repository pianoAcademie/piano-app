...
SOURCE_FIRST_BOOKING_ID = UUID("b9a7ca62-1321-4239-b7db-b89729083593")
TARGET_FIRST_SESSION_ID = UUID("49232bd8-accf-46fc-a2e3-6172e3f23288")
START_DATE, EXPECTED_COUNT = date(2026, 9, 7), 31
...
def count_group(db, group_id):
    return db.scalar(select(func.count()).select_from(Booking).join(CourseSession,CourseSession.id==Booking.session_id).where(Booking.user_id==STUDENT_ID,Booking.status.in_(BOOKING_STATUSES_ACTIVE),CourseSession.recurrence_group_id==group_id,CourseSession.start_at_utc>=datetime(2026,9,7,tzinfo=PARIS).astimezone(timezone.utc)))

def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--apply",action="store_true"); args=parser.parse_args(); now=datetime.now(timezone.utc); start=datetime(2026,9,7,tzinfo=PARIS).astimezone(timezone.utc)
    with SessionLocal() as db:
        ...
        first=db.scalar(select(Booking).where(Booking.id==SOURCE_FIRST_BOOKING_ID).with_for_update()); source_first=db.get(CourseSession,first.session_id) if first and first.user_id==STUDENT_ID else None
        if source_first is None or source_first.recurrence_group_id!=SOURCE_GROUP_ID: raise SystemExit(f"[{SCRIPT}] source_first_guard_failed")
        target_first=db.scalar(select(CourseSession).where(CourseSession.id==TARGET_FIRST_SESSION_ID).with_for_update())
        if target_first is None or target_first.recurrence_group_id!=TARGET_GROUP_ID: raise SystemExit(f"[{SCRIPT}] target_first_guard_failed")
        guard_slot(db,source_first,time(17),"rosana"); guard_slot(db,target_first,time(18),"rym")
        ...
        days=sorted(sb); print(f"[{SCRIPT}] mode={'apply' if args.apply else 'dry-run'} move=31 first={days[0]} last={days[-1]} source=Mon_17_Scheffer_Rosana target=Mon_18_Scheffer_Rym price=keep_source notifications=none")
        ...
        db.add(ClientNoteEntry(user_id=STUDENT_ID,author_user_id=None,entry_type="AUTO",message=f"{SCRIPT} - 31 reservations transferees a compter du 07/09/2026 : lundi 17h Rue Scheffer (Rosana) vers lundi 18h Rue Scheffer (Rym). Tarifs et rattachements financiers conserves. Aucune notification envoyee.")); db.flush()
        ...
        db.commit(); print(f"[{SCRIPT}] committed=true moved=31 source_after=0 target_after={target_after}")
