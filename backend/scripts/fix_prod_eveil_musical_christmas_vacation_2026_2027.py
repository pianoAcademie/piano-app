...
SCRIPT_PREFIX = "PROD_REPAIR_NORAH_ONLINE_PIANO_QUANTITY"
...
def main() -> None:
    ...
    parser.add_argument("--apply", action="store_true", help="Apply the quote line quantity repair.")
    ...
    with SessionLocal() as db:
        ...
        _print(
            "line_update "
            f"quote={quote.quote_number}|line={line.id}|old_quantity={old_quantity}|new_quantity={TARGET_QUANTITY}|"
        ...
        summary = (
            f"apply={args.apply}|quote={quote.quote_number}|line={line.id}|old_quantity={old_quantity}|"
            f"target_quantity={TARGET_QUANTITY}|old_amount_ttc={old_amount_ttc}|target_amount_ttc={new_amount_ttc}|"
            f"target_quote_total_ttc={_q2(Decimal(quote.total_ttc or 0)) if args.apply else '-'}"
        )
        _print(f"summary {summary}")
        print(f"::notice title=Norah online piano quantity repair::{summary}")
