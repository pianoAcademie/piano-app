...
with SessionLocal() as db:
 ...
 profs=list(db.scalars(select(Professor).where((Professor.first_name.ilike('%Estela%'))|(Professor.last_name.ilike('%Estela%')))).all());print('PROFS',[(x.id,x.first_name,x.last_name,x.active) for x in profs])
 ...
 rows=db.execute(select(CourseSession,CourseType,Location,Professor).join(CourseType,CourseType.id==CourseSession.course_type_id).join(Location,Location.id==CourseSession.location_id).outerjoin(Professor,Professor.id==CourseSession.professor_id).where(CourseSession.start_at_utc>=START,CourseSession.start_at_utc<END,Location.city.ilike('Bar-le-Duc')).order_by(CourseSession.start_at_utc)).all()
