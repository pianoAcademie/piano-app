...
class QuoteDocumentMarkupTests(unittest.TestCase):
    ...
    def test_solfege_planning_stays_on_selected_live_series_instead_of_widening_to_june(self) -> None:
        activity_id = uuid4()
        location_id = uuid4()
        ...
        rows += [live_row(iso_date, other_recurrence_id) for iso_date in ["2027-06-01", "2027-06-08", "2027-06-15"]]
        fake_db = SimpleNamespace(
            scalar=lambda _query: None,
            ...
            "blocks": [
                {
                    ...
                    "end_time": "18:20",
                    "modality": "ONLINE",
                    "series_key": str(selected_recurrence_id),
    ...
    def test_solfege_planning_uses_saved_session_count_as_limit_for_same_live_series(self) -> None:
        activity_id = uuid4()
        location_id = uuid4()
        recurrence_id = uuid4()
        paris = ZoneInfo("Europe/Paris")
        ...
        rows = [live_row(iso_date) for iso_date in [*selected_dates, *june_dates]]
        fake_db = SimpleNamespace(
            scalar=lambda _query: None,
            scalars=lambda _query: SimpleNamespace(all=lambda: []),
            execute=lambda _query: SimpleNamespace(all=lambda: rows),
        ...
        self.assertEqual(hydrated["blocks"][0]["sessions_count"], 26)
        self.assertEqual(hydrated["blocks"][0]["planning_session_limit"], 26)
        self.assertEqual(hydrated["blocks"][0]["series_key"], str(recurrence_id))
    ...
    def test_legacy_typeform_planning_block_extends_expected_sessions_after_sparse_live_tail(self) -> None:
        ...
        def live_row(iso_date: str) -> tuple[SimpleNamespace, SimpleNamespace, SimpleNamespace]:
            local_day = date.fromisoformat(iso_date)
            local_start = datetime.combine(local_day, time(16, 0), tzinfo=paris)
            local_end = datetime.combine(local_day, time(17, 0), tzinfo=paris)
            return (
                SimpleNamespace(
                    id=uuid4(),
                    course_type_id=activity_id,
                ...
                SimpleNamespace(id=activity_id, name="Eveil musical", mode="ONSITE"),
                SimpleNamespace(id=location_id, name="Rue de Richelieu", timezone="Europe/Paris", is_online=False),
            )

        live_dates: list[str] = []
        current = date(2026, 9, 23)
        while current <= date(2027, 4, 14):
            live_dates.append(current.isoformat())
            current += timedelta(days=7)
        rows = [live_row(iso_date) for iso_date in live_dates]
        fake_db = SimpleNamespace(
            scalar=lambda _query: None,
            execute=lambda _query: SimpleNamespace(all=lambda: rows),
        )
        ...
        dates = [item["date"] for item in hydrated["sessions"]]
        self.assertEqual(hydrated["sessions_count"], 30)
        self.assertIn("2027-06-16", dates)
        self.assertNotIn("2027-06-23", dates)
        self.assertEqual(hydrated["blocks"][0]["end_date"], "2027-06-16")
    ...
    def test_saturday_planning_block_includes_final_teaching_day(self) -> None:
        activity_id = uuid4()
        location_id = uuid4()
        fake_db = SimpleNamespace(
            scalar=lambda _query: None,
        ...
        snapshot = {
            "blocks": [
                {
                    "activity_id": str(activity_id),
                    "activity_label": "Initiation au piano",
                    "location_id": str(location_id),
                    "location_label": "Rue de la Pompe",
                    "weekday": 5,
        ...
        dates = [item["date"] for item in hydrated["sessions"]]
        self.assertIn("2027-06-19", dates)
