...
function formatEventTime(value: string, language: UiLanguage, timezone?: string): string {
  const parsed = new Date(value);
  if (Number.isNaN(parsed.getTime())) {
    return "--:--";
  }
  return parsed.toLocaleTimeString(language === "en" ? "en-GB" : "fr-FR", {
...
function typeLabelFromKey(typeKey: "PRIVATE" | "ONLINE" | "HOME" | "GROUP", language: UiLanguage): string {
  ...
  if (typeKey === "HOME") {
    return pickText(language, "Domicile", "Home");
  }
...
function defaultTitleFromType(typeLabel: string, language: UiLanguage): string {
  ...
  if (typeKey === "ONLINE") {
    return pickText(language, "Cours online", "Online lesson");
  }
  if (typeKey === "HOME") {
    return pickText(language, "Cours domicile", "Home lesson");
  }
  return pickText(language, "Cours collectif", "Group lesson");
}

function sanitizeTitle(value: string, typeLabel: string, language: UiLanguage): string {
  const trimmed = (value || "").trim();
  if (!trimmed) {
  ...
  if (hasTestPattern) {
    const defaultTitle = defaultTitleFromType(typeLabel, language);
    return defaultTitle === pickText(language, "Cours collectif", "Group lesson")
      ? pickText(language, "Creneau test", "Test slot")
      : defaultTitle;
  }
  return trimmed;
}
...
function localizedStatusLabel(event: PlanningEventChipData, language: UiLanguage): string {
  const normalized = (event.status || "").toUpperCase();
  if (normalized === "CANCELLED") {
    return pickText(language, "Annule", "Cancelled");
  }
  if (normalized === "COMPLETED") {
    return pickText(language, "Termine", "Completed");
  }
  if (normalized === "SCHEDULED" || normalized === "PLANNED") {
    return pickText(language, "Planifie", "Scheduled");
  }
...
export default function MonthEventChip({
  ...
  const teacherCompactBase = !teacherRequired
    ...
    : teacherMissing
      ? pickText(language, "(non renseigne)", "(missing)")
      : compactTeacherName(effectiveTeacherName || teacherFullName);
  const teacherCompact = isSubstituteActive && !teacherMissing
    ? `${teacherCompactBase} ${pickText(language, "(rempl.)", "(sub.)")}`
    : teacherCompactBase;
  const locationLabel = (event.location_label || "").trim() || pickText(language, "Lieu", "Location");
  ...
  const statusLabel = localizedStatusLabel(event, language);
  const tooltip = [
    `${startTime}-${endTime}`,
    displayTitle,
    `${pickText(language, "Prof effectif", "Assigned teacher")}: ${!teacherRequired ? pickText(language, "non requis", "not required") : teacherMissing ? pickText(language, "(non renseigne)", "(missing)") : effectiveTeacherName}`,
    isSubstituteActive && habitualTeacherName ? `${pickText(language, "Prof habituel", "Regular teacher")}: ${habitualTeacherName}` : null,
    isSubstituteActive && substituteTeacherName ? `${pickText(language, "Remplacant", "Substitute")}: ${substituteTeacherName}` : null,
    `${pickText(language, "Lieu", "Location")}: ${locationLabel}`,
    `${pickText(language, "Type", "Type")}: ${typeLabel}`,
    `${pickText(language, "Statut", "Status")}: ${statusLabel}`,
  ]
