  ...
  return (
    <>
      <section className="auth-step-card auth-details-card">
        ...
        <label>
          {requiredLabel(uiText(language, "auth.step_1_subject"))}
          <select
        ...
        <label>
          {requiredLabel(uiText(language, "auth.phone", { suffix: contactLabelSuffix }))}
          <input type="tel" name="phone" required maxLength={30} autoComplete="tel" />
        </label>
        <label>
          {requiredLabel(uiText(language, "auth.postal_address", { suffix: contactLabelSuffix }))}
          <input
            type="text"
            name="address_line"
            required
        ...
        <div className="grid cols-2 config-form-grid">
          <label>
            {requiredLabel(uiText(language, "auth.postal_code", { suffix: contactLabelSuffix }))}
            <input type="text" name="postal_code" required maxLength={20} autoComplete="postal-code" />
          </label>
          <label>
            {requiredLabel(uiText(language, "auth.city", { suffix: contactLabelSuffix }))}
            <input type="text" name="city" required maxLength={120} autoComplete="address-level2" />
          </label>
        </div>
        <label>
          {requiredLabel(uiText(language, "auth.address_country", { suffix: contactLabelSuffix }))}
        ...
        <label>
          {requiredLabel(uiText(language, "auth.residence_country"))}
        ...
        <label>
          {requiredLabel(uiText(language, "auth.password"))}
