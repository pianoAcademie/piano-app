...
function upstreamPublicPayUrl(request: NextRequest, clientId: string, noteId: string): URL {
  const upstreamUrl = new URL(`${backendUrl()}/api/v1/admin/clients/${clientId}/invoices/range/${noteId}/public-pay`);
...
export async function GET(request: NextRequest, { params }: RouteParams): Promise<Response> {
  ...
  const upstream = await fetch(upstreamPublicPayUrl(request, clientId, noteId).toString(), {
    method: "GET",
    cache: "no-store",
    redirect: "manual",
  });
