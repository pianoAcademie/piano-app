...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  const upstream = await fetch(
    `${backendUrl()}/api/v1/teacher/admin/statements/${encodeURIComponent(professorId)}/${year}/${month}/export.csv`,
    {
      method: "GET",
  ...
  if (!upstream.ok) {
    ...
    fallback.searchParams.set("error_code", "statement_export_failed");
  ...
  return new Response(await upstream.arrayBuffer(), {
    ...
    headers: {
      ...
      "content-disposition": upstream.headers.get("content-disposition") ?? `attachment; filename="releve_heures_${year}_${month}.csv"`,
