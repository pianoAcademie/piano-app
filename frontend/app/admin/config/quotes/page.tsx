          ...
          <form action={createAdminQuoteTemplateV2ConfigAction} className="grid cols-2 config-form-grid top-gap-sm">
            ...
            <div className="span-2">
              <QuoteTemplateEditor
                ...
                defaultBody={
                  "{document_style_html}{cover_page_standard_html}{header_standard_html}<h1>Devis {quote_number}</h1>{page_break_html}<h2>Informations famille</h2><div class='quote-block'>{prospect_identity_block_html}</div><h2>Activites</h2>{services_table_html}<h3>Planning detaille des activites</h3>{activities_planning_table_html}<h2>Materiel</h2>{products_table_html}<h2>Kits</h2>{kits_table_html}<h2>Paiement</h2>{payment_method_block_html}{payment_schedule_table_html}<h2>Avoir / Dette</h2>{financial_adjustment_block_html}{solfege_block_html}{masterclass_block_html}{pass_recup_block_html}<h2>Calendrier des cours</h2><p>{calendar_summary}</p>{calendar_activity_semesters_html}<p><strong>Total avant ajustement:</strong> {total_before_adjustment} {currency}</p><p><strong>Total HT:</strong> {total_ht} {currency}</p><p><strong>TVA ({vat_rate}%):</strong> {vat_amount} {currency}</p><p><strong>Total TTC facture:</strong> {total_after_adjustment} {currency}</p><p><strong>Expiration:</strong> {expires_at}</p>{footer_standard_html}"
                }
                    ...
                    <form action={updateAdminQuoteTemplateV2ConfigAction} className="grid cols-2 config-form-grid top-gap-sm">
                      ...
                      <div className="span-2">
                        <QuoteTemplateEditor
                          ...
                          defaultBody={
                            prefill?.body ||
                            "{document_style_html}{header_standard_html}<h1>Devis {quote_number}</h1>{page_break_html}<h2>Informations famille</h2><div class='quote-block'>{prospect_identity_block_html}</div><h2>Activites</h2>{services_table_html}<h3>Planning detaille des activites</h3>{activities_planning_table_html}<h2>Materiel</h2>{products_table_html}<h2>Kits</h2>{kits_table_html}<h2>Paiement</h2>{payment_method_block_html}{payment_schedule_table_html}<h2>Avoir / Dette</h2>{financial_adjustment_block_html}<h2>Calendrier des cours</h2>{calendar_activity_semesters_html}<p>Total TTC facture: {total_after_adjustment} {currency}</p>{footer_standard_html}"
                          }
                          variables={templateVariables}
                        />
                      </div>
