                    ...
                    <form action={updateAdminQuoteTemplateV2ConfigAction} className="grid cols-2 config-form-grid top-gap-sm">
                      ...
                      <div className="span-2">
                        <QuoteTemplateEditor
                          ...
                          defaultBody={
                            prefill?.body ||
                            "{document_style_html}{header_standard_html}<h1>Devis {quote_number}</h1>{page_break_html}<h2>Informations famille</h2><div class='quote-block'>{prospect_identity_block_html}</div><h2>Les Activites retenues</h2>{activities_planning_table_html}<h2>Prestations</h2>{services_table_html}<h2>Materiel</h2>{products_table_html}<h2>Kits</h2>{kits_table_html}<h2>Remises et supplements</h2>{adjustments_table_html}<h2>Paiement</h2>{payment_method_block_html}{payment_schedule_table_html}<h2>Avoir / Dette</h2>{financial_adjustment_block_html}<h2>Calendrier des cours</h2>{calendar_activity_semesters_html}<p>Total TTC facture: {total_ttc_after_adjustment} {currency}</p>{footer_standard_html}"
                          }
                          variables={templateVariables}
                        />
                      </div>
