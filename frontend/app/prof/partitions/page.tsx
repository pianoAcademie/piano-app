...
export default async function Page({ searchParams }: { searchParams: { week?: string; student?: string; session?: string } }) {
  ...
  return <PartitionDistribution key={`${response.data.week}-${searchParams.session ?? ""}`} data={response.data} studentId={searchParams.student} sessionId={searchParams.session} />;
}
