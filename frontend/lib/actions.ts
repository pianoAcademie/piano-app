...
export async function updateAdminClientAction(formData: FormData): Promise<void> {
  ...
  const payload = {
    ...
    last_name: lastName,
    address_line: optionalField(formData, "address_line"),
    home_access_instructions: client_kind === "ADULT" ? optionalField(formData, "home_access_instructions") : null,
    postal_code: optionalField(formData, "postal_code"),
    city: optionalField(formData, "city"),
    address_country,
    mobile_phone_1: optionalField(formData, "mobile_phone_1"),
...
export async function adminViewTeacherPortalAction(formData: FormData): Promise<void> {
  ...
  const result = await backendRequest<AdminImpersonationStartOut>(
    `/api/v1/admin/impersonate/teacher/${teacherId}?view_mode=${viewMode}`,
    {
      method: "POST",
    },
    token,
...
export async function createAdminClientAction(formData: FormData): Promise<void> {
  ...
  const payload = {
    ...
    home_access_instructions: isChildClient ? null : optionalField(formData, "home_access_instructions"),
    postal_code: optionalField(formData, "postal_code"),
    city,
    address_country,
    mobile_phone_1: optionalField(formData, "mobile_phone_1"),
      ...
      const createAdultResult = await backendRequest<{ id: string }>(
        "/api/v1/admin/clients",
        {
          ...
          body: JSON.stringify({
            ...
            home_access_instructions: optionalField(formData, "adult_home_access_instructions"),
  ...
  const result = await backendRequest<{ quote: { id: string } }>(
    `/api/v1/quotes/${encodeURIComponent(quoteId)}/send`,
    {
      ...
      body: JSON.stringify({
        ...
        confirm_missing_kit: checkboxFieldWithDefault(formData, "confirm_missing_kit", false),
      }),
    },
    token,
  );
...
export async function createQuoteVariantAction(formData: FormData): Promise<void> {
  const token = currentToken();
  if (!token) {
    redirect("/login?error_code=session_expired");
  }
  ...
  const result = await backendRequest<{ quote: { id: string } }>(
    `/api/v1/quotes/${encodeURIComponent(quoteId)}/duplicate?as_variant=true`,
    { method: "POST" },
    token,
  );
  ...
  revalidatePath(`/admin/quotes/${quoteId}`);
  const detailPath = withUiLanguage(`/admin/quotes/${encodeURIComponent(result.data.quote.id)}?back=${encodeURIComponent(successReturnTo)}`, language);
  redirect(withUiMessageCode(detailPath, "ok", "quote_variant_created", { lang: language }));
}
